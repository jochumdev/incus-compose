...
// TestForwardIsGone pins what makes this build the light one. Naming the flag
// is an error rather than a no-op, so a command line carried over from
// cmd/coredns is refused at startup instead of quietly refusing, later, every
// name it used to forward.
func TestForwardIsGone(t *testing.T) {
	cmd := runCommand()
	cmd.Action = func(context.Context, *cli.Command) error { return nil }

	err := cmd.Run(t.Context(), []string{"run", "--forward", "10.0.0.1:53"})
	require.Error(t, err)
