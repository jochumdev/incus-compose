...
// it.
func (c *Connection) GetInstanceConsoleLog(ctx context.Context, project string, name string) (io.ReadCloser, error) {
	return c.doRaw(ctx, project, http.MethodGet, incusInstancePath(name, "/console"), nil)
}
...
func (c *Connection) ConsoleInstance(ctx context.Context, project string, name string, console api.InstanceConsolePost, args *InstanceConsoleArgs) (<-chan api.Operation, error) {
	...
	updates, err := c.asyncOperation(ctx, project, http.MethodPost, incusInstancePath(name, "/console"), console, "")
	if err != nil {
		return nil, err
	}
	...
	_, err = c.dialOperation(streamCtx, project, started.ID, consoleSecret(fds, api.SecretNameControl))
	if err != nil {
		closeStreams()
	...
	socket, err := c.dialOperation(streamCtx, project, started.ID, consoleSecret(fds, "0"))
	if err != nil {
		closeStreams()
