...
func labeled(project, name string, instance, own map[string]string) *shared.Event {
	...
	return shared.NewEvent(time.Now(), "instance-updated", project, name, "").
		WithInstance(true, instance, map[string]*shared.Network{project + "/net0": net}).
		WithProject(own)
}
...
func TestDistillJoinsTheProjectUnderTheInstance(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			instance: map[string]string{labelPrefix + metaZone: "own.example."},
			own:      map[string]string{labelPrefix + metaZone: "shop.example."},
			zone:     "own.example.",
		},
		{
		...
		},
		{
			...
			instance: map[string]string{labelPrefix + metaService: "api"},
		...
		},
		{
			...
			instance: map[string]string{labelPrefix + metaZone: "  "},
			own:      map[string]string{labelPrefix + metaZone: "shop.example."},
			zone:     "shop.example.",
		},
	}
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			inst := distill(labeled("shop", "web", test.instance, test.own), nil, "incus")
...
func TestProjectZoneOverridesTheGeneratedOne(t *testing.T) {
	...
	held := map[string]*instance{
		"shop/web": distill(labeled("shop", "web", nil, own), nil, "incus"),
		"shop/db":  distill(labeled("shop", "db", nil, own), nil, "incus"),
	}
...
func TestInstanceLabelsPreferCompose(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			config: map[string]string{LabelServiceCompose: "web"},
			want:   "web",
		},
		{
			name: "compose wins where both are set",
			config: map[string]string{
				LabelServiceCompose:       "web",
				labelPrefix + metaService: "api",
			},
			want: "web",
		},
		{
		...
		},
		{
			...
			config: map[string]string{labelPrefix + metaService: "api"},
			want:   "api",
		},
		{
...
func TestProjectAliasesAreNotInherited(t *testing.T) {
	...
	own := distill(labeled("shop", "web",
		map[string]string{labelPrefix + metaAliases: "store"},
