...
func TestVisibility(t *testing.T) {
	...
	t.Run("sharedNetworkAddress", s.testSharedNetworkAddress)
	...
	t.Run("ptr", s.testPTR)
	t.Run("respondsToInstanceStop", s.testRespondsToInstanceStop)
}
...
func (s *stack) ask(ctx context.Context, t *testing.T, mode queryMode, from, qname, qtype string) (string, error) {
	...
	args := []string{"kdig", "+timeout=3", "+retry=0"}
...
// testSharedNetworkAddress is the assertion a naive implementation fails:
// users-db is only on users-net, so it gets users-api's address there.
func (s *stack) testSharedNetworkAddress(t *testing.T) {
	ctx, cancel := context.WithTimeout(t.Context(), commandTimeout)
	defer cancel()
...
func (s *stack) testIdentifiesQuerierBySourceAddress(t *testing.T) {
	...
	out, err := s.exec(ctx, "gateway", "kdig", "@"+s.dnsAddr(), "-p", corednsPort,
...
func (s *stack) testPTR(t *testing.T) {
	...
	assert.Containsf(t, out, "users-db-1."+s.zone(), "the reverse must name the instance:\n%s", out)
...
// testRespondsToInstanceStop checks the store follows the Incus event stream
// rather than a timer.
//
...
func (s *stack) testRespondsToInstanceStop(t *testing.T) {
	stopCtx, cancel := context.WithTimeout(t.Context(), commandTimeout)
	defer cancel()

	_, err := s.compose(stopCtx, "stop", "products-db")
	require.NoError(t, err, "stopping products-db")

	// Safety net for the failure path below, where the inline restart is skipped.
	t.Cleanup(func() {
		if !t.Failed() {
			return
		}
		...
		_, err := s.compose(ctx, "start", "products-db")
		assert.NoError(t, err, "restarting products-db")
	})

	require.Eventually(t, func() bool {
		out, err := s.ask(stopCtx, t, viaResolver, "products-api", "products-db."+s.zone(), "A")

		return err == nil && rcode(out) == "NXDOMAIN"
	}, 60*time.Second, 2*time.Second, "products-db still resolved after being stopped")

	startCtx, cancel := context.WithTimeout(t.Context(), commandTimeout)
	defer cancel()

	_, err = s.compose(startCtx, "start", "products-db")
	require.NoError(t, err, "restarting products-db")

	// Wait for the record to come back, so the rest of the suite sees a whole
	// stack. This doubles as the other half of the event-stream assertion.
	require.Eventually(t, func() bool {
		out, err := s.ask(startCtx, t, viaResolver, "products-api", "products-db."+s.zone(), "A")

		return err == nil && rcode(out) == "NOERROR" && answerCount(out) > 0
	}, 90*time.Second, 2*time.Second, "products-db never resolved again after restarting")
}

// rcode extracts the response code from a kdig header. Matching the bare string
// is unsafe: "NOERROR" also appears in the EDNS pseudosection's ext-rcode.
func rcode(out string) string {
	for _, line := range strings.Split(out, "\n") {
		_, after, found := strings.Cut(line, "status: ")
		if !found {
			continue
		}
...
func answerCount(out string) int {
	for _, line := range strings.Split(out, "\n") {
		_, after, found := strings.Cut(line, "ANSWER: ")
		if !found {
			continue
		}
...
// answerAddress returns the first address in a kdig answer section.
...
// containsAny reports whether out mentions any of the given addresses.
