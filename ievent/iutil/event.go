// Package iutil is the vocabulary the source and its plugins both speak: the
...
type Event struct {
	...
	// instanceLabels and projectLabels are kept apart by source rather than
	// merged: a consumer that must tell which of the two actually set a key -
	// a project-only setting that must never be forged by an instance's own
	// config, say - reads one or the other. Label and Labels fold them
	// together on read for a consumer that just wants the effective value.
